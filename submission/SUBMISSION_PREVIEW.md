...
const isPandigital = (input) => {
    ...
    if (typeof input === 'number') {
        // Numbers smaller than 1023456789 cannot be 0-9 pandigital.
        // This also serves as a guard against `MAX_SAFE_INTEGER` issues for numbers
        // that are too small to contain all digits.
        if (input < 1023456789) return false;
        const strVal = String(input);
        if (strVal.includes('e')) return false; // Scientific notation implies precision loss or non-integer.
        return checkStringBitmask(strVal);
    }

    const str = String(input);
    // [DESIGN RATIONALE] We remove the `str.length < 10` guard here.
    // The formal definition of pandigital means "contains all digits at least once",
    // not "is a permutation of 0-9". A number like 10234567890 is pandigital.
    // The bitmask check handles this naturally.
...
2. **Mathematical Correctness:** By supporting inputs longer than 10 digits, we adhere to the formal definition of pandigital numbers (containing "at least one" of each digit), rather than just detecting permutations.
